    ...
    for global_step in range(start_step, max_steps):
        ...
        if config.train_dataset.distributed_load:
            ...
            if rank == 0:
                ...
                with stats_tracker.record_timing("rollout"):
                    if config.async_training:
                        batch = rollout.prepare_batch(train_dataloader, workflow=workflow, distributed_load=False)
                    else:
                        try:
        ...
        if not config.train_dataset.distributed_load:
            batch = distributed_batch(batch, rank, config.train_dataset.batch_size, device, world_size, config.train_dataset.balance_batch)
        else:
