...
from areal.utils.data import distributed_batch
...
def main(args):
    ...
    if config.train_dataset.distributed_load:
        train_dataset = get_boba_math_dataset(config.train_dataset.path, tokenizer, rank=rank, world_size=world_size)
    else:
        train_dataset = get_boba_math_dataset(config.train_dataset.path, tokenizer, rank=0, world_size=1)
    
    if config.train_dataset.distributed_load or rank == 0:
    ...
    if not config.train_dataset.distributed_load:
    ...
    for global_step in range(start_step, max_steps):
        ...
        if config.train_dataset.distributed_load or rank == 0:
            ...
            with stats_tracker.record_timing("rollout"):
                if config.async_training:
                    batch = rollout.prepare_batch(train_dataloader, workflow=workflow, distributed_load=config.train_dataset.distributed_load)
                else:
                    try:
        ...
        if not config.train_dataset.distributed_load:
            batch = distributed_batch(batch, rank, config.train_dataset.batch_size, device, world_size)
        else:
        ...
        with stats_tracker.record_timing("checkpoint_for_recover"):
            recover_handler.dump(
                ...
                train_dataloader,
                tokenizer=tokenizer,
                distributed_load=config.train_dataset.distributed_load,
            )
