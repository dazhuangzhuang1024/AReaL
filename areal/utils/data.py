...
def distributed_batch(batch: List[TensorDict], rank, batch_size, device, world_size, balance_batch_enabled):
    if rank == 0:
...
def balance_batch(batch: List[TensorDict], world_size) -> List[TensorDict]:
    ...
    return repad_tensordict_inplace_for_dp_rank(batches)

def repad_tensordict_inplace_for_dp_rank(batches: List[TensorDict]):
    attention_mask_key = 'attention_mask'
    for tensordict in batches:
