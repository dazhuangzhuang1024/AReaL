...
vLLMEngine currently only supports single-controller. Cannot be used in SPMD
"""
...
class vLLMEngine(InferenceEngine):
    ...
    # FIXME vllm adapt, guess vllm offline api
    async def agenerate(self, req: ModelRequest) -> ModelResponse:
        """Async version of generate using local vllm engine."""
        if not hasattr(self, "engine") or self.engine is None:
            raise RuntimeError(
                "Local vLLM engine is not initialized, cannot generate."
            )
        ...
        if gconfig.n_samples != 1:
            raise ValueError(
                "LocalvLLMEngine does not support n_samples > 1. "
        ...
        while (
            ...
            try:
                ...
                raise RuntimeError(f"Local vLLM engine generation failed: {e}")
    ...
    def _update_weights(self, meta: WeightUpdateMeta):
        if not hasattr(self, "engine") or self.engine is None:
            raise RuntimeError(
                "Local vLLM engine is not initialized, cannot update weights."
            )
        if meta.type == "disk":
            try:
